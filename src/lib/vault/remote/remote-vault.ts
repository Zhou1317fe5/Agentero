...
import {
	...
	type SshConfigHost_Serialize,
} from "@/lib/core/bindings";
import { callApiResult } from "@/lib/core/ipc";
import { readJsonStorage, writeJsonStorage } from "@/lib/core/storage";
import { isTauri } from "@/lib/core/tauri";

export type { FsCaps, RemoteSessionInfo };
export type RemoteDirEntry = FsDirEntry;
...
export async function remoteConnect(args: {
	...
	return callApiResult(() => commands.remoteConnect(args), {
...
export async function remoteSshConfigHosts(): Promise<SshConfigHost[]> {
	if (!isTauri()) return [];
	const hosts = await callApiResult(() => commands.remoteSshConfigHosts());
	return hosts.map((host: SshConfigHost_Serialize) => ({
		alias: host.alias,
...
export async function remoteEnsureVault(
	...
	return callApiResult(
		() => commands.remoteVaultEnsure({ sessionId, locale: locale ?? null }),
		{
...
export async function remoteDisconnect(sessionId: string): Promise<void> {
	if (!isTauri()) return;
	await callApiResult(() => commands.remoteDisconnect({ sessionId }), {
...
export async function remoteList(
	...
	return callApiResult(() => commands.remoteList({ sessionId, path }), {
...
export async function remoteReadText(
	...
	return callApiResult(() => commands.remoteReadText({ sessionId, path }), {
...
export async function remoteWriteText(
	...
	await callApiResult(
		() => commands.remoteWriteText({ sessionId, path, content }),
		{
...
export async function remoteMkdir(
	...
	await callApiResult(() => commands.remoteMkdir({ sessionId, path }), {
...
export async function remoteRemove(
	...
	await callApiResult(
		() => commands.remoteRemove({ sessionId, path, recursive }),
		{
...
export async function remoteWriteBytes(
	...
	await callApiResult(
		() =>
			commands.remoteWriteBytes({ sessionId, path, data: Array.from(data) }),
		{
...
export async function remotePaperList(sessionId: string): Promise<unknown[]> {
	return callApiResult(() => commands.remotePaperList({ sessionId }), {
...
export async function remotePaperRescan(
	...
	return callApiResult(() => commands.remotePaperRescan({ sessionId }), {
...
export async function remoteAgentScan(
	...
	return callApiResult(() => commands.remoteAgentScan({ sessionId }), {
...
export async function remoteAgentProbe(
	...
	return callApiResult(
		() => commands.remoteAgentProbe({ sessionId, templateId }),
		{
...
export async function remoteAgentOpenInstallTerminal(
	...
	await callApiResult(
		() => commands.remoteAgentOpenInstallTerminal({ sessionId, templateId }),
		{
...
export async function remoteCacheFile(
	...
	const r = await callApiResult(
		() => commands.remoteCacheFile({ sessionId, path }),
		{
...
export async function remoteCacheStats(
	...
	return callApiResult(
		() => commands.remoteCacheStats({ sessionId: sessionId ?? null }),
		{
...
export async function remoteCacheClear(
	...
	return callApiResult(
		() => commands.remoteCacheClear({ sessionId: sessionId ?? null }),
		{
