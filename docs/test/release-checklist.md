...
| 11.1.10 | 右栏·Agent（支持 Client terminal 的 Agent） | 依次执行 `pwd`、`echo AGENTERO_SMOKE`，再启动长命令并在超时后 kill | 前两条及时返回；长命令等待期间 output 可读，kill / release 不阻塞，卡片最终 completed / failed | ☐ |
| 11.1.11 | 右栏·Agent | 在工具卡 pending 时结束或取消回合，再接收一条迟到 completion | 回合结束后不保留永久 spinner；迟到状态按同一 toolCallId 修正原卡片，不新增重复卡片 | ☐ |
