...
| # | 界面 | 操作 | 预期 | 结果 |
|---|---|---|---|---|
| 7.2.1 | 中间栏·PDF | 拖选一段文字 | 仅选中文字显示平滑蓝色选区 + 操作菜单（高亮 / 加入对话 ⌘K / 快速对话 / 翻译，无批注按钮；文字小号）+ 页右缘竖向评论入口；不出现整页 / 大块 PDF 蓝色矩形框选 | ☐ |
...
| 7.2.7 | 划词加入对话 | 划词 → 点 **加入对话** 或按 `⌘L` / `⌘K` → 在 Agent 输入问题发送 | Composer 有选区 chip 且侧栏打开；发送后选区旁出现**对话卡片**页边针（`kind: ask`，非视觉批注）；悬停/点开可见本轮问答 | ☐ |
