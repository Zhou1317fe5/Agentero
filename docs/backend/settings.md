...
- GitHub 镜像（`githubMirrorEnabled` / `githubMirrorBaseUrl`）：Skill 导入专用 URL 前缀回退（直连 `api.github.com` / `codeload.github.com` 失败后再试 `{base}/https://…`）。默认关；不内置公益镜像列表。与网络代理正交。见 [skill-import.md](skill-import.md)。
