...
- **选区上下文**：Markdown/PDF 选中文字 → 瞬时选区 chip（虚线）；`⌘L` / `⌘K` / 划词工具栏「加入对话」固定为 chip 并打开 Agent；「快速对话」打开页内 Ask。发送时以引用块消费；PDF 带几何的选区在发送后插入 `kind: ask` 对话卡片页边针（非视觉批注）。
...
`⌘P` 快速打开（论文 + `vault_search` 全文）；`⇧⌘P`/`>` 执行内置命令。`⌘K` 将选区加入 Agent 对话。
