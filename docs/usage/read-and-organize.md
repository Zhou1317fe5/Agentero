...
1. 在 PDF 中拖选一段文本。
2. 在选区菜单中选择颜色，创建高亮。
3. 需要写解释时，把指针移到页右缘对应高度的竖向评论入口（hover 进入编辑；移走且没输入则缩回竖向卡片；输入后提交），带评论的高亮会常驻在页右缘评论列，点击卡片即可修改；窗口较窄时回退为页边针。
4. 右侧 **批注** 面板可列出当前 PDF 的提问与视觉批注（跳转 / 删除）。
...
已保存的纯视觉批注会出现在页右缘评论列，点击卡片即可就地修改备注；若该批注已有 Agent 会话，页边针仍可打开对话。也可在笔记中用 `[[论文@id]]` 或 `![[论文@id]]` 引用。
...
- 卡片下方是**引用近邻图**：当前论文、本文引用的库内论文、引用本文的库内论文、未入库 stub 用不同形状区分。点击库内节点打开对应论文；header 图标可切到全库引用边。这与 Markdown 双链反链不是同一张图。
...
提问会把选中文本和页码作为上下文，结果写入 `marks/`。划词提问使用设置中为该能力配置的 Agent，不一定与右侧 Agent 面板当前会话相同。需已配置可用的 BYOA Agent。

## 翻译

1. 选中原文。
2. 选择 **翻译**。
3. 按设置使用 **Agentero 内置**服务、免费机器翻译或 BYOA Agent。
4. 查看并保留需要的翻译记录。
