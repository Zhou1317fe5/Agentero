...
- 调度：每个输入一个 JobCenter `import` job（Normal lane；并发取 `batchImportConcurrency`；`params` = `mode` + 来源标识并参与去重，同一 URL 不重复入队）。编排仍在渲染端执行器（`src/lib/paper/import/import-tasks.ts` 按 `params.mode` 分发），job id 兼作 Host `task_id`，字节进度与协作取消复用同一条链路。
