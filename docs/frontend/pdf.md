...
选区后浮动工具栏：高亮 / 复制 / 提问 / 加入对话 / 翻译。**批注**不在工具栏里：选区出现时页右缘评论列会在对应高度出现一条竖向入口（与空评论卡同高、宽度更窄，内为评论图标），hover 展开成评论卡（移走指针缩回），点击或聚焦输入框后创建高亮并进入就地编辑（空内容取消会按 `isNew` 清掉草稿）。Settings → 翻译开启「划词自动翻译」后，选区文本提取完成即自动启动翻译并打开结果卡；关闭时保留手动翻译入口。
