...
PDFium 的 `page.size` 已经包含页面 `/Rotate`（例如竖向 A4 上 `/Rotate 90` 的宽表，尺寸是横向的，光栅也是转正后的）。滚动布局和瓦片不能再按 `page.rotation` 对调宽高或重排瓦片，否则页框变回竖向、表格被多转 90°。阅读器自己的旋转（`coreDoc.rotation`）仍然叠加在这个显示尺寸上。划词菜单同样不再按 `/Rotate` 反向旋转。
