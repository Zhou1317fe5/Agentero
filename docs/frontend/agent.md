...
- 运行中可继续输入 → Queue waitlist（「等待发送」）；标题保持简洁，条目等宽并可单独移除；Esc / 停止中止。队列**浮在**输入框上方（绝对定位、叠在 transcript 底部），不占用也不改动 composer 固定高度与位置，避免紧凑一行模式下把输入框挤掉或顶替；紧凑模式下队列自身也会收紧 padding。
...
- 引用上下文：`@` 提及与 `$skill` 在输入正文里**行内插入**小 pill（contenteditable；行高贴近正文、`max-w` 较短；点击或 Backspace 整颗删除）。草稿以 `{{m:path}}` / `{{s:skillId}}` marker 持久化；发送时从气泡正文剥离 marker，路径 / skillIds 仍走原有 context 与 Host 通路。当前文件、选区、视觉批注仍为输入框上方的块级 chip；图片附件仍在边框内。紧凑一行模式下块级 chip 与图片收成图标圆片；隐藏底部工具栏，圆形向上箭头发送按钮与输入框同一行（无内容时置灰）。右侧栏 composer 顶部有竖向拖拽分隔条，可压低输入区高度。
